...
class MongoTechoPropioRepository(TechoPropioRepository):
    ...
    def _create_indexes(self) -> None:
        ...
        try:
            ...
            self.collection.create_index(
                ...
                unique=False,  # CAMBIO: No único, permite múltiples solicitudes del mismo DNI
    ...
    async def save_application(self, application: TechoPropioApplication) -> TechoPropioApplication:
        ...
        try:
            ...
            # Permitir múltiples solicitudes del mismo DNI para casos legítimos
            # (ej: solicitud rechazada, nueva solicitud, etc.)
            ...
            try:
                result = await self.collection.insert_one(document)
                application.id = str(result.inserted_id)
                logger.info(f"Solicitud guardada con timestamp único: {application.id}")
                return application
    ...
    async def check_dni_exists_in_applications(
        ...
        try:
            ...
            count = await self.collection.count_documents(query)  # ✅ Agregar await
            return count > 0
    ...
    def _document_to_entity(self, document: Dict[str, Any]) -> TechoPropioApplication:
        ...
        application.created_by = document.get("created_by", "system")  # ✅ Asignar después de la creación
    ...
    def _property_info_to_dict(self, property_info: PropertyInfo) -> Dict[str, Any]:
        ...
        return {
            ...
            "address": property_info.address,  # ✅ AGREGADO: Campo address faltante
    ...
    def _dict_to_property_info(self, data: Dict[str, Any]) -> PropertyInfo:
        ...
        property_info = PropertyInfo(
            ...
            lote=data["lote"],
            address=data["address"],  # ✅ AGREGADO: Campo address requerido
    ...
    def _household_member_to_dict(self, member: HouseholdMember) -> Dict[str, Any]:
        ...
        return {
            ...
            "civil_status": member.civil_status.value,  # ✅ FIX: Campo agregado
            "education_level": member.education_level.value,
            "occupation": member.occupation,  # ✅ FIX: Campo agregado
            "employment_situation": member.employment_situation.value,  # ✅ FIX: Campo agregado
            "work_condition": member.work_condition.value,  # ✅ FIX: Campo agregado
            "monthly_income": float(member.monthly_income),  # ✅ FIX: Campo agregado
            "disability_type": member.disability_type.value,
            "relationship": member.relationship.value if member.relationship else None,
    ...
    def _dict_to_household_member(self, data: Dict[str, Any]) -> HouseholdMember:
        ...
        member = HouseholdMember(
            ...
            document_number=data["document_number"],
            birth_date=datetime.fromisoformat(data["birth_date"]).date(),
            civil_status=CivilStatus(data["civil_status"]),  # ✅ FIX: Campo agregado
            education_level=EducationLevel(data["education_level"]),
            occupation=data["occupation"],  # ✅ FIX: Campo agregado
            employment_situation=EmploymentSituation(data["employment_situation"]),  # ✅ FIX: Campo agregado
            work_condition=WorkCondition(data["work_condition"]),  # ✅ FIX: Campo agregado
            monthly_income=Decimal(str(data["monthly_income"])),  # ✅ FIX: Campo agregado
